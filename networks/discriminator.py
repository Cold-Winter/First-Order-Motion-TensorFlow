...
from utils.blocks import DownBlock2dWoadv 
from utils.blocks import Conv2DSpectralNorm
class Discriminator(tf.keras.Model):
  ...
  def call(self, x, key_points):
    ...
    out = tf.concat([out, tf.transpose(heatmap, [0, 2, 3, 1])], axis=-1)
...
class DiscriminatorWoadv(tf.keras.Model):
  def __init__(self):
    super(DiscriminatorWoadv, self).__init__()
    self.num_channels = 3
    self.num_keypoints = 10
    self.num_blocks = 4
    self.num_features_list = [64, 128, 256, 512] 
    ...
    for i in range(self.num_blocks):
      ...
      norm = i != 0
      pool = i != self.num_blocks - 1
      encoder_blocks.append(DownBlock2dWoadv(num_features, norm, pool))
    
    self.encoder_blocks = encoder_blocks
    self.conv = Conv2DSpectralNorm(1, kernel_size=1)
